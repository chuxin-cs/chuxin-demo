    ...
    offsetMap({ keyProp, itemSize, sizes, listData }) {
      ...
      for (let i = 0; i < listData.length; i++) {
        ...
        if (typeof key === "undefined") {
          this.warn &&
            console.warn(
              `data[${i}][${keyProp}] 为 undefined，请确保 keyProp 对应的值不为undefined`
            );
        }
        res[key] = total;
  ...
  methods: {
    ...
    initData() {
      ...
      if (!this.elTable || this.elTable.$options.name !== "ElTable") {
        throw new Error(
          "未找到 <el-table> 组件. 请确保 <el-table> 组件在虚拟组件内，且 getElTable 方法能获取到正确的 <el-table> 组件！"
        );
      }
    ...
    updateSizes() {
      if (!this.dynamic) return;
      let rows = this.$el.querySelectorAll(
      ...
      if (isTree) {
        ...
        rows = Array.from(
          this.$el.querySelectorAll(".el-table__body > tbody > .el-table__row")
        ).filter((row) => {
      ...
      Array.from(rows).forEach((row, index) => {
        ...
        if (isTree) {
          ...
          while (
            next &&
            next.tagName === "TR" &&
            noFirstLevelReg.test(next.className)
          ) {
    ...
    calcRenderData() {
      ...
      const bottom =
        getScrollTop(scroller) + getOffsetHeight(scroller) + buffer - toTop;
      ...
      if (
        this.start === 0 &&
        this.end > 30 &&
        this.end === this.listData.length - 1
      ) {
        this.warn &&
          console.warn(
            "[el-table-virtual-scroll] 表格数据全部渲染，渲染数量为:" +
    ...
    updateSelectionByRowKey(data, oldData = []) {
      ...
      this.oldSelection = this.oldSelection.filter(
        (row) => !(row[this.keyProp] in usedMap)
      );
    },

    // 【多选】多选列是否设置了 reserve-selection
    isReserveSelection() {
      return this.columnVms.some(
        (vm) => vm.reserveSelection && vm.isSelection()
      );
    },
    ...
    hackRowHighlight() {
      ...
      if (this.elTable.setCurrentRow.virtual) {
        const originSetCurrentRow = this.elTable.setCurrentRow.bind(
          this.elTable
        ); // 原方法
      ...
      const unWatch = this.$watch(
        () => this.elTable.currentRowKey,
        (val) => {
          if (this.elTable.rowKey) {
            const targetRow = this.listData.find(
              (row) => val === row[this.elTable.rowKey]
            );
    ...
    cellFixedStyle({ column }, isHeader = false) {
      ...
      if (
        ...
        for (let i = 0; i < columns.length; i++) {
          ...
          const isLeft =
            column.className &&
            column.className.includes("virtual-column__fixed-left");
          const isRight =
            column.className &&
            column.className.includes("virtual-column__fixed-right");
        ...
        // 设置左边、右边固定列class
        if (lastLeftColumn && !lastLeftColumn.className.includes(leftClass))
          lastLeftColumn.className += leftClass;
        if (
          firstRightColumn &&
          !firstRightColumn.className.includes(rightClass)
        )
          firstRightColumn.className += rightClass;
      ...
      if (elTable.showSummary) {
        this.$nextTick(() => {
          const footTh = this.$el.querySelector(
            `.el-table__footer-wrapper .${column.id}`
          );
          if (footTh) {
    ...
    bindTableDestory() {
      const onTableDestory = () => {
        this.warn &&
          console.warn(
            "<el-table> 组件销毁时，建议将 <el-table-virtual-scroll> 组件一同销毁"
          );
        this.destory();
        this.$nextTick(() => {
          this.initData();
